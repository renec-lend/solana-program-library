...
use std::{convert::TryFrom, convert::TryInto, mem::size_of};
...
pub enum LendingInstruction {
    ...
    ///   2. `[]` Pyth Reserve liquidity oracle product account.
    ///             Must be the Pyth product account specified at InitReserve.
    ///   3. `[]` Switchboard Reserve liquidity oracle account.
    ...
    ///   6. `[]` Clock sysvar (optional, will be removed soon).
    ///   7. `[]` Token program id.
    ///   8.  `[]` RepayDebtVoucher Account, PDA generated by nft voucher that store the details about the voucher
    ///   9. `[writable]` nft_mint, the NFT voucher address
    ///  10. `[writable]` nft_token_account: The user token account that own the nft_mint
    ///  11. `[]` Pyth Reserve liquidity oracle account.
    ///             Must be the Pyth price account specified at InitReserve.
    ///  12. `[]` Pyth Reserve liquidity oracle product account.
    ///             Must be the Pyth product account specified at InitReserve.
    ///  13. `[]` Switchboard Reserve liquidity oracle account.
    ///             Must be the Switchboard price feed account specified at InitReserve.
    RepayObligationLiquidityWithNFTVoucher {
...
impl LendingInstruction {
    ...
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        Ok(match tag {
            ...
            24 => {
                let (liquidity_amount, _rest) = Self::unpack_u64(rest)?;
                Self::RepayObligationLiquidityWithNFTVoucher { liquidity_amount }
            }
            _ => {
...
pub fn refresh_reserve(
    ...
    reserve_liquidity_pyth_product_pubkey: Pubkey,
    ...
    let mut accounts = vec![
        ...
        AccountMeta::new_readonly(reserve_liquidity_pyth_product_pubkey, false),
