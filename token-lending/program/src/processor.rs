...
use crate::{VOUCHER_PROGRAM_AUTHORITY, VOUCHER_PROGRAM_ID};
use bytemuck::bytes_of;
use oracles::{get_single_price, validate_pyth_alike_keys};
...
use solend_sdk::state::RepayDebtVoucher;
use solend_sdk::{
    math::SaturatingSub,
...
pub mod solend_market_owner {
    solana_program::declare_id!("AaJy5CcKS8S8oyFHDiU2voNz9sTSM4GyyWzcuXE42QvT");
}
...
pub fn process_instruction(
    ...
    match instruction {
        ...
        LendingInstruction::RepayObligationLiquidityWithNFTVoucher { liquidity_amount } => {
            process_repay_obligation_liquidity_with_discount(program_id, liquidity_amount, accounts)
        }
    }
}
...
fn process_init_reserve(
    ...
    validate_pyth_alike_keys(pyth_price_info, pyth_product_info)?;
...
#[inline(never)] // avoid stack frame limit
fn process_repay_obligation_liquidity_with_discount(
    program_id: &Pubkey,
    liquidity_amount: u64,
    accounts: &[AccountInfo],
) -> ProgramResult {
    ...
    let repay_debt_voucher = next_account_info(account_info_iter)?;
    let nft_mint = next_account_info(account_info_iter)?;
    let nft_token_account = next_account_info(account_info_iter)?;
    let pyth_oracle = next_account_info(account_info_iter)?;
    let pyth_product = next_account_info(account_info_iter)?;
    let switchboard_oracle = next_account_info(account_info_iter)?;
    ...
    if *repay_debt_voucher.owner != VOUCHER_PROGRAM_ID {
        msg!("Repay debt voucher is not owned by voucher program");
        return Err(LendingError::InvalidVoucherProgramOwner.into());
    }

    let repay_data = match RepayDebtVoucher::try_from_account(repay_debt_voucher) {
        Ok(r) => r,
        Err(_) => {
            msg!("Error deserializing RepayDebtVoucher");
            return Err(LendingError::InvalidAccountInput.into());
        }
    };

    if repay_data.discriminator != RepayDebtVoucher::discriminator() {
        msg!("RepayDebtVoucher discriminator mismatched");
        return Err(LendingError::InvalidVoucherDiscriminator.into());
    }

    if repay_data.authority != VOUCHER_PROGRAM_AUTHORITY {
        msg!("RepuayDebtVoucher authority is not correct");
        return Err(LendingError::InvalidVoucherAuthority.into());
    }

    if repay_data.market != *lending_market_info.key {
        msg!("RepayDebtVoucher market is not correct");
        return Err(LendingError::InvalidVoucherMarket.into());
    }

    if !repay_data.is_universal_discount && !repay_data.eligible_tokens.contains(&repay_reserve.liquidity.mint_pubkey)
    {
        msg!("RepayDebtVoucher is not applicable to this token");
        return Err(LendingError::InvalidVoucherEligibleToken.into());
    }

    let current_time = Clock::get()?.unix_timestamp as i64;
    if current_time < repay_data.start_time || current_time > repay_data.end_time {
        msg!("RepayDebtVoucher is not applicable for this time");
        return Err(LendingError::InvalidVoucherTimeRange.into());
    }
    ...
    let mut interest_amount = repay_reserve
        .liquidity
        .cumulative_borrow_rate_wads
        .try_div(liquidity.cumulative_borrow_rate_wads)?;
    interest_amount = if interest_amount < Decimal::one() {
        Decimal::one().try_sub(interest_amount)?
    } else {
        interest_amount.try_sub(Decimal::one())?
    };

    interest_amount = interest_amount.try_mul(liquidity.borrowed_amount_wads)?;
    let discount_rate = Decimal::from_percent(repay_data.discount_percentage as u8);
    let mut interest_amount_after_discount = interest_amount.try_mul(discount_rate)?;

    let (market_price, _) = get_price(Some(switchboard_oracle), pyth_oracle, pyth_product, clock)?;
    let discount_cap = Decimal::from(repay_data.maximum_amount * ((10u64).pow(repay_reserve.liquidity.mint_decimals as u32)));
    let interest_amount_after_discount_in_market_price =
        interest_amount_after_discount.try_mul(market_price)?;
    if interest_amount_after_discount_in_market_price > discount_cap {
        let max_cap_discount = discount_cap.try_div(market_price)?;
        interest_amount_after_discount = interest_amount.try_sub(max_cap_discount)?;
    } else {
        interest_amount_after_discount = interest_amount.try_sub(interest_amount_after_discount)?;
    }

    let liquidity_with_full_interest = Decimal::from(liquidity_amount)
        .try_add(interest_amount)?
        .try_floor_u64()?;
    let liquidity_with_discount = Decimal::from(liquidity_amount)
        .try_add(interest_amount_after_discount)?
        .try_floor_u64()?;
    ...
    let CalculateRepayResult {
        ...
        .calculate_repay(liquidity_with_full_interest, liquidity.borrowed_amount_wads)?;
    ...
    let actual_repay_amount = min(repay_amount, liquidity_with_discount);
...
/// `get_price` attempts to retrieve the oracle price from Renec oracle first if the provided main_price_account_info is belong to Renec oracle.
/// If it fails, it falls back to Pyth.
/// If Pyth also fails, it then falls back to the Switchboard if provided.
/// The first element in the returned tuple is the market price, and the second is an optional
/// smoothed price (e.g., EMA, TWAP).
fn get_price(
    secondary_price_account_info: Option<&AccountInfo>,
    main_price_account_info: &AccountInfo,
    main_product_account_info: &AccountInfo,
    clock: &Clock,
) -> Result<(Decimal, Option<Decimal>), ProgramError> {
    if let Ok(prices) = get_single_price(main_price_account_info, main_product_account_info, clock) {
    ...
    if let Some(secondary_price_account_info_unwrapped) = secondary_price_account_info {
        ...
        if let Ok(prices) = get_single_price(secondary_price_account_info_unwrapped, main_product_account_info, clock) {
            return Ok((prices.0, prices.1));
        }
    }
